...
init_price = 30000.0
number_trades = 10
percentage = 1.01  # Процент за нивата на цените
profit_percentage = (percentage ** 4 + percentage ** 3) / 2  # Определя процентът печалба от чакащите количества на база
# като средноаритметичното между цените на третата и четврътата поред транзацкии.
busd_quantity = [844, True]  # True или False определя дали бусд е главна валута
btc_quantity = [0]
...
print(initial_qty_busd)
print(initial_qty_btc)
print(overall_busd)
print(overall_btc)
...
def print_func(exec_info):
    ...
    total_busd = busd_quantity[0] + btc_quantity[0]* trading_price[0]
    total_btc = btc_quantity[0] + busd_quantity[0]/trading_price[0]
    ...
    if asset == 'BUSD':
        ...
        minimal_price = sum(traded_btc_qty) / busd_quantity[0]
    ...
    if asset_on_charge == 'BUSD':
        if is_valid_price:
            ...
            if len(list_traded_prices) == 0:
                list_traded_prices.append(trading_price)
                ...
                if trading_price > list_traded_prices[-1]:
                    ...
                    list_traded_prices.appendleft(trading_price)
...
while True:
    curr_price = float(input("Въведи текуща цена: "))
