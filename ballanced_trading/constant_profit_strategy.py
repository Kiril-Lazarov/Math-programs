...
class Print_info:
    ...
    def execution_print(self, asset, curr_trade_qty, new_qty):
        first = ''
        second = ''
        next_profit_price = 0
        print()
        ...
        return f'{curr_trade_qty}  {first} -> {new_qty} {second} на цена {curr_price:.2f}\n' \
               f'Следваща цена за частична печалба: {next_profit_price:.2f}'

    def get_overall_quantities(self):
        btc.btc_overall = buffer.calculate_overall('BTC', curr_price)
        busd.busd_overall =buffer.calculate_overall('BUSD', curr_price)
        return f'Общо количество BTC: {btc.btc_overall:.8f}\nНачално количество BTC: {init_btc_overall_qty:.8f}\n' \
               f'Общо количество BUSD: {busd.busd_overall:.2f}\nНачално количество BUSD: {init_busd_overall_qty:.2f}'

    def print_profits(self, trades_count, asset):
        return f'Обща печалба BTC: {(btc.btc_overall / btc.base_overall - 1) * 100:.2f}%\n' \
               f'Обща печалба BUSD: {(busd.busd_overall / busd.base_overall - 1) * 100:.2f}%\n' \
               f'Поредна транзакция: {asset}: {trades_count}\n' \
               f'Следваща горна цена: {btc.next_price:.2f}\n'\
               f'Следваща долна цена: {busd.next_price:.2f}'


    def profit_execution(self, asset, waiting_qty, profit_qty):
        next_price = 0
        temp_asset = ''
        if asset == 'BTC':
        ...
        return f'\n-------ЧАСТИЧНА ПЕЧАЛБА--------\n\n{waiting_qty} {temp_asset} -> {profit_qty} {asset} на цена: {curr_price:.2f}\n' \
               f'Следваща цена за частична печалба: {next_price:.2f}'
...
class General:
    ...
    def create_ground_price(self, price):
        return price

    def create_new_price(self, asset, price):
        if asset == 'BTC':
            return price * price_percentage
        else:
            return price / price_percentage

    def check_curr_price(self, curr_price):
    ...
    def execution_order(self, asset, curr_price):
        trades_count = 0
        if asset == 'BTC':
    ...
    def check_for_profit(self, asset):
        trades_count = 0
        is_profit = False
        if asset == 'BTC':
    ...
    def check_for_total_profit(self):
        ...
        if (curr_total_btc/btc.base_overall-1) * 100 >= 0.02 and (curr_total_busd/ busd.base_overall -1) * 100>=0.02:
            ...
            print(f'\n-------ТОТАЛНА ПЕЧАЛБА-------\nНова базова цена: {ground_price:.2f}\nТотална печалба BTC:'
                  f' {(btc.base_overall/init_btc_overall_qty - 1) *100}%\nНово активно количество BTC: {btc.quantity})\n'
                  f'Ново активно количество BUSD: {busd.quantity}\nСледваща горна цена: {btc.next_price}\n'
                  f'Следваща долна цена: {busd.next_price}\nПечалба BTC: {(curr_total_btc / btc.base_overall - 1) * 100:.2f}%\n'
                  f'Печалба BUSD: {(curr_total_busd / busd.base_overall - 1) * 100:.2f}%')
...
buffer = General()
print_buffer = Print_info()
ground_price = float(client.get_symbol_ticker(symbol='BTCBUSD')['price'])
initial_btc = 0.02651
initial_busd = 844

# The variable buffer is used only to get access to the class General's methods
...
print('Processing...')
print(f'Следваща горна цена: {btc.next_price}')
print(f'Следваща долна цена: {busd.next_price}')
print(f'Начално количество BTC: {init_btc_overall_qty}')
print(f'Начално количество BUSD: {init_busd_overall_qty}')
while True:
