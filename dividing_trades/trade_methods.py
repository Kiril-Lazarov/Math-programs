class TradeMethods:
    ...
    def check_for_split(self, btc_total_init, btc_qty, btc_reached_profit, busd_total_init, busd_qty,
        ...
        return False, btc_reached_profit, busd_reached_profit
    ...
    def partial_split(self, curr_price, btc_qty, busd_qty, asset, factor):
        print(f'\n-------ЧАСТИЧНО РАЗДЕЛЯНЕ-------')
        print(f'\nТекуща цена: {curr_price:.2f}')
        if asset == 'BTC':
            qty = btc_qty * factor
            btc_qty -= qty
            busd_qty += qty *curr_price

        else:
            qty = busd_qty * factor
            busd_qty -= qty
            btc_qty += qty/curr_price
        return btc_qty, busd_qty
